...
public class PriorityQueue<TElement, TPriority> {
    ...
    /// <summary>
    ///     Checks if the specified element exists in the <see cref="PriorityQueue{TElement, TPriority}" />.
    /// </summary>
    /// <param name="element">The element to search for in the <see cref="PriorityQueue{TElement, TPriority}" />.</param>
    public bool ContainsKey(TElement element)
    {
        if(element == null){ return false; }
        foreach ((TElement,TPriority) v in _nodes)
        {
            if (element.Equals(v.Item1))
            {
                return true;
            }
        }
